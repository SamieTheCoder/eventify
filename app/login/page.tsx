  ...
  return (
    <div className="flex min-h-screen flex-col bg-zinc-950">
      <header className="flex items-center justify-between px-6 pt-6 sm:px-8">
        <Link href="/" className="inline-flex items-center gap-1.5 text-[15px] font-semibold tracking-tight text-zinc-100 transition-colors hover:text-zinc-300">
      ...
      <main className="flex flex-1 items-center justify-center px-4 py-12">
        <div className="w-full max-w-sm space-y-7">
          ...
          {/* Heading */}
          <div>
            <h1 className="text-3xl font-bold tracking-tight text-zinc-50">Login</h1>
            <p className="mt-1.5 text-sm text-zinc-400">Welcome back to Eventify</p>
          </div>
            ...
            <div className="space-y-4">
              <div className="rounded-xl border border-emerald-800/40 bg-emerald-950/30 px-4 py-4 text-center">
                ...
                <p className="mt-1 text-xs text-zinc-400">
                  Login link sent to <span className="font-medium text-zinc-300">{email}</span>
                </p>
              </div>
              {error && <p className="rounded-lg bg-red-950/40 px-3 py-2 text-sm text-red-400">{error}</p>}
              <button
              ...
              <p className="text-center text-xs text-zinc-600">
                {resendCount === 0 ? 'Can resend after 15s' : 'Subsequent resends take 30s'}
              </p>
              <button
                type="button"
                onClick={() => { setMagicSent(false); setError(''); setResendCount(0); setCooldown(0) }}
          ...
          <p className="text-center text-sm text-zinc-500">
            ...
            <Link href="/signup" className="font-semibold text-zinc-100 underline-offset-4 hover:underline">
              Sign up
            </Link>
          </p>
